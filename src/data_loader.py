...
def load_data(args):
    ...
    user_triple_sets, user_neighbor_triple_sets = kg_propagation(args, kg, user_init_entity_set,
                                                                 args.user_triple_set_size, True)
    logging.info("contructing items' kg triple sets ...")
    item_triple_sets, item_neighbor_triple_sets = kg_propagation(args, kg, item_init_entity_set,
                                                                 args.item_triple_set_size, False)
    return train_data, eval_data, test_data, n_entity, n_relation, user_triple_sets, item_triple_sets, user_neighbor_triple_sets, item_neighbor_triple_sets
...
def kg_propagation(args, kg, init_entity_set, set_size, is_user):
    ...
    neighbor_triple_sets = collections.defaultdict(list)  
    for obj in init_entity_set.keys():
        ...
        for l in range(n_layer):
            ...
            all_h, all_r, all_t = [], [], []  
            if l == 0:
                entities = init_entity_set[obj]  # E0o
            else:
            ...
            for entity in entities:
                for tail_and_relation in kg[entity]:
                    ...
                    all_h.append(entity)  # ** modify
                    all_t.append(tail_and_relation[0])
                    all_r.append(tail_and_relation[1])

            # if len(h) == 0:
            if len(all_h) == 0:  
                triple_sets[obj].append(triple_sets[obj][-1])
                neighbor_triple_sets[obj].append(neighbor_triple_sets[obj][-1])
            else:
                # indices = np.random.choice(len(h), size=set_size, replace=(len(h) < set_size))
                # h = [h[i] for i in indices]
                # r = [r[i] for i in indices]
                # t = [t[i] for i in indices]
                indices = np.random.choice(len(all_h), size=set_size, replace=(len(all_h) < set_size))  
                h = [all_h[i] for i in indices]
                r = [all_r[i] for i in indices]
                t = [all_t[i] for i in indices]
                triple_sets[obj].append((h, r, t))  
                neighbor_triple_sets[obj].append(
                    _get_neighbor_triple_sets(args.neighbor_size, all_h, all_r, all_t, indices)) 
    return triple_sets, neighbor_triple_sets
